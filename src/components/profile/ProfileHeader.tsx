...
export default function ProfileHeader({userName, avatar, signature, onUpdate}: Props) {
  ...
  const pickImage = async () => {
    ...
    try {
      ...
      formData.append('file', {uri, name: asset.fileName || 'avatar.jpg', type: asset.type || 'image/jpeg'} as any);
  ...
  const saveField = async (patch: {avatar?: string; signature?: string}) => {
    ...
    if (res.code === 200 && res.data) {
      const info = {id: res.data.id, userName: res.data.userName, avatar: res.data.avatar, signature: res.data.signature};
      setUserInfo(info);
      ...
      {editing ? (
        <View style={styles.editWrap}>
          ...
          <View style={styles.actions}>
            ...
            <Pressable onPress={() => {setEditing(false); setDraft(signature);}}>
...
const styles = StyleSheet.create({
  ...
  name: {marginTop: agriTheme.spacing.md, fontSize: 20, fontWeight: '700', color: agriTheme.colors.textMain},
  signature: {marginTop: agriTheme.spacing.xs, fontSize: 14, color: agriTheme.colors.textSecondary, textAlign: 'center'},
